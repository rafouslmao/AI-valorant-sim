...
function applyRatingOverride(player, seedPlayer = {}) {
  const o = seedPlayer.ratingOverride;
  if (!o) return;
  if (o.attrs && typeof o.attrs === 'object') {
    player.attrs = { ...player.attrs, ...o.attrs };
  }
  if (o.attributes && typeof o.attributes === 'object') {
    player.attributes = {
      ...player.attributes,
      ...o.attributes,
      mechanics: { ...player.attributes.mechanics, ...(o.attributes.mechanics || {}) },
      utilitySkill: { ...player.attributes.utilitySkill, ...(o.attributes.utilitySkill || {}) },
      decisionMaking: { ...player.attributes.decisionMaking, ...(o.attributes.decisionMaking || {}) },
      mental: { ...player.attributes.mental, ...(o.attributes.mental || {}) },
      teamplay: { ...player.attributes.teamplay, ...(o.attributes.teamplay || {}) },
      physical: { ...player.attributes.physical, ...(o.attributes.physical || {}) }
    };
  }
  if (Number.isFinite(o.operatorAim)) {
    player.attributes.mechanics.operatorAim = clamp(Math.round(o.operatorAim), 20, 100);
  }
  if (Array.isArray(o.traitHints) && o.traitHints.length) {
    player.traits = [...new Set([...(player.traits || []), ...o.traitHints])].slice(0, 4);
  }
}
...
function createImportedPlayer(seedPlayer) {
  ...
  applyRatingOverride(player, seedPlayer);
...
// 3) Per-player random attribute overrides are generated in src/core/database.js for imported players.
