import { FACILITY_CONFIG, MAP_POOL, PRACTICE_FOCUS, ROLES, SECONDARY_ROLE_TAGS, TEAMS } from './constants.js';
...
function genAttributes() {
  return {
    aim: randInt(50, 95), utility: randInt(45, 95), clutch: randInt(40, 95), mental: randInt(50, 95), teamwork: randInt(45, 95), decisionMaking: randInt(40, 90)
  };
}
...
function pickSecondaryTag() {
  return SECONDARY_ROLE_TAGS[randInt(1, SECONDARY_ROLE_TAGS.length - 1)];
}

function roleMarketScarcity(role) {
  return role === 'Controller' ? 1.08 : role === 'Initiator' ? 1.04 : 1;
}
...
export function createPlayer(tid) {
  ...
  const salary = Math.round(randInt(20_000, 60_000) * roleMarketScarcity(primaryRole));
  const player = {
    pid: uid('p'), tid, name: genName(), age: randInt(17, 29), salary,
    reputation: randInt(35, 90), ambition: randInt(20, 95), loyalty: randInt(20, 95), greed: randInt(20, 95), playtimeDesire: randInt(30, 95),
    preferredRole: primaryRole,
    attrs: genAttributes(),
    roles: [...new Set([primaryRole, secondaryRole])],
    currentRole: primaryRole,
    secondaryRoleTag: pickSecondaryTag(),
    roleSkills,
    ...
    currentContract: {
      ...
      yearsRemaining: randInt(1, 3),
      signedWithTid: tid,
      buyoutClause: Math.round(salary * randInt(2, 5)),
      rolePromise: randInt(0, 100) > 35 ? 'starter' : 'bench',
      signingBonus: Math.round(salary * 0.15)
    },
    history: []
  };
...
function coreCoachRatings() {
  return {
    prep: randInt(40, 95), mapPool: randInt(40, 95), vetoSkill: randInt(40, 95), compCrafting: randInt(40, 95), midSeriesAdapt: randInt(40, 95),
    practiceDesign: randInt(40, 95), skillDevelopment: randInt(40, 95), roleDevelopment: randInt(40, 95), talentID: randInt(40, 95),
    leadership: randInt(40, 95), discipline: randInt(40, 95), cultureFit: randInt(40, 95), conflictMgmt: randInt(40, 95),
    timeoutValue: randInt(40, 95), clutchControl: randInt(40, 95), composure: randInt(40, 95), riskBalance: randInt(40, 95)
  };
}
...
export function createCoach(tid = null, role = 'Head Coach') {
  return {
    ...
    profile: { name: genName(), age: randInt(28, 58), nationality: nationalities[randInt(0, nationalities.length - 1)], styleTag: coachStyles[randInt(0, coachStyles.length - 1)] },
...
function mapRatings() {
  return Object.fromEntries(MAP_POOL.map((m) => [m.id, randInt(45, 85)]));
}
...
function createTeam(teamInfo) {
  return {
    ...
    monthlyLedger: [], facilities: initFacilities(), mapRatings: mapRatings(),
